...
const broken = 'may vary.`})]})]})}`}function'
const fixed = 'may vary.`})]})]})]})}`}function'
...
for (const slug of readdirSync(publicDir, { withFileTypes: true })
  ...
  if (!src.includes(broken)) {
    console.log(`MISS ${slug}`, JSON.stringify(src.slice(src.indexOf('may vary.'), src.indexOf('may vary.') + 50)))
    continue
  }
  ...
  try {
    ...
    console.log(`BAD ${slug}`, JSON.stringify(src.slice(src.indexOf('may vary.'), src.indexOf('may vary.') + 50)))
  }
}
